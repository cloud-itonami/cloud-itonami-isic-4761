(ns bookstoreops.sim
  ...
  the permanently-excluded return/refund-finalization or content-
  inclusion/exclusion-editorial-finalization scope."
...
(defn- approve! [actor tid]
  (g/run* actor {:approval {:status :approved :by "bookstore-inventory-coordinator-1"}} {:thread-id tid :resume? true}))

(defn -main [& _]
  (let [db (store/seed-db)
        coordinator-phase-1 {:actor-id "coord-1" :actor-role :bookstore-coordinator :phase 1}
        coordinator-phase-3 {:actor-id "coord-1" :actor-role :bookstore-coordinator :phase 3}
        actor (op/build db)]
    ...
    (let [r (exec-op actor "t1" {:op :log-sales-record :store-id "store-1"
                                  :patch {:units-sold 58 :returns 2 :stock-count-delta -60}} coordinator-phase-1)]
      (println r)
      (println "-- human bookstore inventory coordinator approves --")
    ...
    (println (exec-op actor "t2" {:op :log-sales-record :store-id "store-1"
                                  :patch {:units-sold 34 :returns 1 :stock-count-delta -35}} coordinator-phase-3))
    ...
    (println (exec-op actor "t3" {:op :schedule-staffing-operation :store-id "store-1"
                                  :patch {:shift "weekend-register" :date "2026-07-20" :window "10:00-18:00"}} coordinator-phase-3))
    ...
    (println (exec-op actor "t4" {:op :coordinate-supply-order :store-id "store-1"
                                  :patch {:item "front-list hardcover restock" :quantity 40 :estimated-cost 360.0
    ...
    (let [r (exec-op actor "t5" {:op :coordinate-supply-order :store-id "store-1"
                                 :patch {:item "back-to-school stationery bulk order" :quantity 600 :estimated-cost 2400.0
                                         :vendor-id "vendor-1"}} coordinator-phase-3)]
      (println r)
      (println "-- human bookstore inventory coordinator reviews & approves --")
    ...
    (let [r (exec-op actor "t6" {:op :flag-inventory-concern :store-id "store-1"
                                 :patch {:concern "carton #4471 arrived water-damaged and the packing slip claims 40 units but only 33 were received" :confidence 0.9}} coordinator-phase-3)]
      (println r)
      (println "-- human bookstore inventory coordinator reviews & approves --")
    ...
    (println (exec-op actor "t9" {:op :coordinate-supply-order :store-id "store-1"
                                  :patch {:item "imported specialty notebooks" :quantity 25 :estimated-cost 300.0
    ...
    (let [actor-direct (op/build db {:advisor (reify advisor/Advisor
      ...
      (println (exec-op actor-direct "t10" {:op :schedule-staffing-operation :store-id "store-1"
                                           :patch {:shift "weekday-register" :date "2026-07-22"}} coordinator-phase-3)))

    (println "\n== log-sales-record store-1, advisor drifts into return/refund-finalization scope -> HARD hold, permanent ==")
