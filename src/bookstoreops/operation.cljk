(ns bookstoreops.operation
  "OperationActor -- one coordination request = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor
  (BookstoreRetailAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the BookstoreRetailGovernor (:govern)
  ...
  human store operator/bookstore-inventory coordinator. The approver
  resumes with `{:approval {:status :approved}}` (or :rejected)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [bookstoreops.advisor :as advisor]
            [bookstoreops.governor :as governor]
            [bookstoreops.phase :as phase]
...
(defn build
  ...
  `bookstoreops.store/Store`).
  opts:
    :advisor      -- a `bookstoreops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; BookstoreRetailAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; BookstoreRetailGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
