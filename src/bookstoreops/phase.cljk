(ns bookstoreops.phase
  "Phase 0->3 staged rollout for the ISIC-4761 'Retail sale of books,
  newspapers and stationary in specialized stores' (bookstore/newsstand/
  stationery retail) operations-coordination actor.
  ...
  needs a human to actually look at it. `bookstoreops.governor`'s own
  `always-escalate-ops` enforces the same invariant independently -- two
  layers, not one, agree on this."
  (:require [bookstoreops.governor :as governor]))
...
(defn verdict->disposition
  "Map a BookstoreRetailGovernor verdict to a base disposition before the
  phase gate."
  [verdict]
