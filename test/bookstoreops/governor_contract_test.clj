(ns bookstoreops.governor-contract-test
    ...
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t2"
                               {:op :flag-inventory-concern :store-id "store-1"
                                :patch {:concern "water-damaged carton on receipt" :confidence 0.99}}
                               ctx)]
      (is (some? result))
    ...
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t2b"
                               {:op :coordinate-supply-order :store-id "store-1"
                                :patch {:item "back-to-school stationery bulk order" :quantity 600 :estimated-cost 2200.0
                                        :vendor-id "vendor-1"}}
                               ctx)]
      (is (some? result))
      (is (= 0 (count (store/coordination-log db)))
...
(deftest scope-excluded-content-hard-hold
  (testing "proposal drifting into return/refund-finalization scope -> permanent hard hold"
    (let [db (store/seed-db)
          actor (op/build db)
