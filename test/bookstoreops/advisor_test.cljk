(ns bookstoreops.advisor-test
  "Unit tests of `bookstoreops.advisor` proposal generation."
...
(deftest propose-sales-record-shape
  (testing "sales-record proposal has correct shape and fields"
    (let [p (adv/infer db {:op :log-sales-record
                           :store-id "store-1"
                           :patch {:units-sold 58 :returns 2 :stock-count-delta -60}})]
...
(deftest propose-inventory-concern-shape
  (testing "inventory-concern proposal always escalates"
    (let [p (adv/infer db {:op :flag-inventory-concern
                           :store-id "store-1"
                           :patch {:concern "water-damaged carton on receipt"}})]
